...
class GameManager(object):
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            if self._next_action == ActionState.BISCO:
                ...
                self.AboveHand()
                ret = self.oneShootAction(GripperNumber.GRIPPER1)
                if ret is True:
                    ret = self.oneShootAction(GripperNumber.GRIPPER2)
                if ret is None:
                    self._next_action = ActionState.FINISH
                self._next_action = ActionState.BISCO
    ...
    def biscoAction(self):
        ...
        if is_twin:
            ret = self.normalGo2Bisco(GripperNumber.GRIPPER1, target_ids[0])
            ###### grip both
            ...
            self._gripping_bisco_id[GripperNumber.GRIPPER1] = target_ids[0]
            self._mymoveit.graspBisco(
            ...
            self._gripping_bisco_id[GripperNumber.GRIPPER2] = target_ids[1]
            ...
            if target_ids[1] is not None:
                ret = self.normalGo2Bisco(GripperNumber.GRIPPER2, target_ids[1])
                self._mymoveit.graspBisco(
                    GripperNumber.GRIPPER2,
                    self._biscos.getName(target_ids[1]),
                    True,
