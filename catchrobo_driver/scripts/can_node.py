...
command_set = {
    ...
    'set_self._node_id': (0x006, [('self._node_id', 'I', 1)]), # tested
...
def command(bus, self._node_id_, extended_id, cmd_name, **kwargs):
    ...
    msg = can.Message(arbitration_id=((self._node_id_ << 5) | cmd_id), extended_id=extended_id, data=data)
    bus.send(msg)

async def record_messages(bus, self._node_id, extended_id, cmd_name, timeout = 5.0):
    """
    ...
    try:
        ...
        while True:
            ...
            if ((msg.arbitration_id == ((self._node_id << 5) | cmd_id)) and (msg.is_extended_id == extended_id) and not msg.is_remote_frame):
...
async def request(bus, self._node_id, extended_id, cmd_name, timeout = 1.0):
    cmd_spec = command_set[cmd_name]
    cmd_id = cmd_spec[0]

    msg_generator = record_messages(bus, self._node_id, extended_id, cmd_name, timeout)

    msg = can.Message(arbitration_id=((self._node_id << 5) | cmd_id), extended_id=extended_id, data=[], is_remote_frame=True)
    bus.send(msg)
...
class TestSimpleCAN():
    ...
    #def run_test(self, odrive: ODriveComponent, self._canbus: CanInterfaceComponent, self._node_id: int, extended_id: bool, logger: Logger):
	...
	#axis.clear_errors()
	#axis.config.can_self._node_id = self._node_id
	#axis.config.can_self._node_id_extended = extended_id
	#time.sleep(0.1)
	def __init__(self):
		self._canbus = can.Bus("can0", bustype="socketcan")
		self._node_id = 0x01

	def my_cmd(self,cmd_name, **kwargs): command(self._canbus.handle, self._node_id, cmd_name, **kwargs)
	def my_req(self,cmd_name, **kwargs): return asyncio.run(request(self._canbus.handle, self._node_id, cmd_name, **kwargs))
	def fence(self): self.my_req('get_vbus_voltage') # fence to ensure the CAN command was sent

	my_req('get_vbus_voltage')['vbus_voltage'], odrive.handle.vbus_voltage, accuracy=0.01)

	my_cmd('set_self._node_id', self._node_id=self._node_id+20)
	asyncio.run(request(self._canbus.handle, self._node_id+20, extended_id, 'get_vbus_voltage'))
	test_assert_eq(axis.config.can_self._node_id, self._node_id+20)

	# Reset node ID to default value
	command(self._canbus.handle, self._node_id+20, extended_id, 'set_self._node_id', self._node_id=self._node_id)
	fence()
	test_assert_eq(axis.config.can_self._node_id, self._node_id)
	...
	heartbeats = asyncio.run(get_all(record_messages(self._canbus.handle, self._node_id, extended_id, 'heartbeat', timeout = 1.0)))
