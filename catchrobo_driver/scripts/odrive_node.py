...
class ODriveNode(object):
    ...
    def reboot(self):
        ...
        try:
            ...
            rospy.logerr("Rebooted ODrive.")
        except:
            rospy.logerr("Failed to reboot")
        finally:
            self.driver = None
        return True
    ...
    def calibrate(self, axis=0, mode=0):
        ...
        rospy.loginfo("Vbus %.2fV" % self.driver.vbus_voltage)
        rospy.loginfo("Calibrating axis %d..." % axis)
        ...
        if mode:
            ...
            if self.axes[axis].error != 0:
                rospy.logerr("Failed calibration with axis error 0x%x, motor error 0x%x" % (self.axes[axis].error, self.axes[axis].motor.error))
                return False
