...
class PositionConverter:
    def __init__(self, joint_offset, MOTOR_NUM=5):
        ...
        # reductionRatio[4]=6.0は第四・五アクチュエータベルトにおける、根本と第三関節間の減速比
    ...
    def convert_motor_to_joint(self, motor_state):
        ...
        joint_state_without_offset[4] = (-motor_state.position[3] - motor_state.position[4]) / self.__reductionRatio[6] / self.__reductionRatio[6] + listAffectedDiff[4]
    ...
    def convert_joint_to_motor(self, joint_state):
        ...
        motor_state.position[2] = (joint_state.position[2] - self.__jointOffset[2] - listAffectedDiff[2])
        # TODO: 第四関節・第五関節
        listAffectedDiff[3] = -joint_state.position[1] / self.__reductionRatio[4] - joint_state.position[2] / self.__reductionRatio[5]
        motor_state.position[3] = 0 # TODO
        motor_state.position[4] = 0 # TODO
        # 角度情報を回転位相情報に変換
        motor_state.position = self.convert_rad_to_round(motor_state.position)
        return motor_state