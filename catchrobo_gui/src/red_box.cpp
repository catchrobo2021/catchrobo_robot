#include "field/red_box.h"
#include "ui_red_box.h"
...
namespace shootbox
{
red_box::red_box(QWidget *parent) :
    rviz::Panel(parent),
    ui(new Ui::red_box)
{
    ui->setupUi(this);
    ...
    std::string img_path = "/img/red_box.png";
    ...
    std::string img_on_path = "/img/blue_color_2.png";
    std::string img_off_path = "/img/blue_bw_2.png";
    std::string icon_on_path_str = project_path+img_on_path;
    std::string icon_off_path_str = project_path+img_off_path;
    icon_on_path = QString::fromLocal8Bit(icon_on_path_str.c_str());
    icon_off_path = QString::fromLocal8Bit(icon_off_path_str.c_str());
    ...
    sub_ = n.subscribe("goal_sub", sendtime, &red_box::arrayback, this);
    sub2_ = n.subscribe("highlight_goal", sendtime, &red_box::arrayback2, this);
...
red_box::~red_box() = default;

void red_box::onInitialize()
{
...
void red_box::check_goal()
{
  ...
  icon_on.addFile(this->icon_on_path, QSize(128,128), QIcon::Normal, QIcon::Off);
  icon_off.addFile(this->icon_off_path, QSize(128,128), QIcon::Normal, QIcon::Off);
  for(int i=1; i<7; i++){
    for(int j=1; j<5; j++){
      if(this->box[i-1]>j-1){
...
void red_box::onEnable()
{
  show();
  parentWidget()->show();
}

void red_box::onDisable()
{
  hide();
  parentWidget()->hide();
}

void red_box::click1_up(){
  box[0] += 1;
  this->check_goal();
}
void red_box::click1_dn(){
  box[0] -= 1;
  this->check_goal();
}
void red_box::click2_up(){
  box[1] += 1;
  this->check_goal();
}
void red_box::click2_dn(){
  box[1] -= 1;
  this->check_goal();
}
void red_box::click3_up(){
  box[2] += 1;
  this->check_goal();
}
void red_box::click3_dn(){
  box[2] -= 1;
  this->check_goal();
}
void red_box::click4_up(){
  box[3] += 1;
  this->check_goal();
}
void red_box::click4_dn(){
  box[3] -= 1;
  this->check_goal();
}
void red_box::click5_up(){
  box[4] += 1;
  this->check_goal();
}
void red_box::click5_dn(){
  box[4] -= 1;
  this->check_goal();
}
void red_box::click6_up(){
  box[5] += 1;
  this->check_goal();
}
void red_box::click6_dn(){
  box[5] -= 1;
  this->check_goal();
}
...
void red_box::arrayback2(const std_msgs::Int32MultiArray& msg){
  // print all the remaining numbers
...
PLUGINLIB_EXPORT_CLASS(shootbox::red_box, rviz::Panel )
